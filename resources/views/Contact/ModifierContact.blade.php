...
<div class="row">
	...
	<div class="col-sm-6">
		<div class="card" style="max-width:75rem;margin-left:auto;margin-right:auto;">
			<h2 class="card-header">Modification d'un contact</h2>
			<div class="card-body">
				{{ Form::model($contact,['route' => ['ContactMAJ',$contact->id], 'method' => 'put']) }}
				<div id="nat" class="form-group" style="margin-top:10px;">
					{{ Form::label('contactAMIO', 'Contact au sein d\'AMIO :') }}
					{{ Form::text('contactAMIO',null, ['class' => 'form-control']) }}
				...
				<div class="form-group" style="margin-top:10px;">
					...
					{{ Form::select('interlocuteur', $interlocuteurs ,$contact->interlocuteur_id,['class' => 'form-control']) }}
				</div>
				<div class="form-group" style="margin-top:10px;">
	...
	<div class="col-sm-3">
			...
			{{ link_to_route('FicheContact', 'Annuler',['id'=>$contact->id],['class' => 'btn btn-danger pull-right', 'style' => 'margin-top:10px;margin-right:25px;height:2.5rem;width:55%;margin-bottom:15px;' ]) }}
		{{ Form::close() }}
	</div>
</div>
@endsection
